...
def path(graph, source, target, excluded_edges=None, ooc_types=ooc_types):
    ...
    with without_edges(graph, excluded_edges) as g:
        ...
        # 参考 https://networkx.github.io/documentation/latest/release/migration_guide_from_1.x_to_2.0.html
        edge = graph.edges # graph.edge -> graph.edges
        def path_part(src, tgt):
            node = edge[src,tgt] # node = edge[src][tgt]
...
def without_edges(g, edges):
    ...
    held = dict()
    for a, b in edges:
        held[(a, b)] = g.edges[a, b] # g.edge[a][b] -> g.edges[a, b]
        g.remove_edge(a, b)
